...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    admin_otp_bypass INTEGER NOT NULL DEFAULT 0
);
...
def init_db():
    ...
    if "admin_approved" not in user_cols:
        conn.execute("ALTER TABLE users ADD COLUMN admin_approved INTEGER NOT NULL DEFAULT 0")
        # Existing verified students keep their current access; newly registered
        # students must pass both email verification and admin approval.
        conn.execute("UPDATE users SET admin_approved = verified WHERE role = 'student'")
        conn.commit()
    # Backward-compatible task enhancements; never reset existing data.
    task_cols = {r[1] for r in conn.execute("PRAGMA table_info(tasks)").fetchall()}
    if "task_code" not in task_cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN task_code TEXT")
    ...
    if cur.fetchone() is None:
        conn.execute(
            """INSERT INTO users (full_name, email, phone, student_id, password_hash,
                role, verified, admin_approved, trust_score, created_at)
               VALUES (?,?,?,?,?,?,?,?,?,?)""",
            (
                "Platform Admin",
                ADMIN_EMAIL,
...
def broadcast_to_all_students(subject, message):
    """Send an admin announcement to every student via in-app notifications and email."""
    conn = get_conn()
    users = conn.execute("SELECT id, email, full_name FROM users WHERE role='student' ORDER BY id").fetchall()
    conn.close()
    sent = failed = 0
    for u in users:
        notify(u["id"], f"📢 {subject}: {message}")
        if EMAIL_CONFIGURED:
            body = f"Hi {u['full_name']},\n\n{message}\n\nThis announcement was sent by UNI HELP administration.\n- UNI HELP"
...
def register_user(full_name, email, phone, student_id, password):
    ...
    if "@" not in email or email.startswith("@") or email.endswith("@"):        return False, "Please enter a valid email address."
    if len(password) < 6:
    ...
    conn.execute(
        """INSERT INTO users (full_name, email, phone, student_id, password_hash, role,
            verified, admin_approved, trust_score, created_at)
           VALUES (?,?,?,?,?,?,?,?,?,?)""",
        (full_name.strip(), email, phone.strip(), student_id.strip(),
         hash_password(password), "student", 0, 0, 50, now_iso()),
    )
...
def login_user(email, password):
    ...
    if row["role"] == "student" and not row["verified"]:
        return False, "Email verification is pending. Please verify your email before logging in."
    if row["role"] == "student" and not row["admin_approved"]:
        return False, "Your account is awaiting UNI HELP admin approval. Please wait for approval."
...
def seed_demo_data():
    ...
    for full_name, local, phone, sid in DEMO_STUDENTS:
        ...
        conn.execute(
            """INSERT INTO users (full_name, email, phone, student_id, password_hash, role,
                verified, admin_approved, trust_score, unicoins, created_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (full_name, email, phone, sid, hash_password("demo1234"), "student", 1, 1, 60, 50, now_iso()),
        )
...
<style>
:root{--uh-navy:#102a43;}
.stApp{background:radial-gradient(circle at 10% 10%,rgba(255,255,255,.96),rgba(228,239,255,.92) 42%,rgba(255,239,218,.94) 100%);color:var(--uh-navy);}
.block-container{padding-top:2rem;padding-bottom:4rem;}
.uh-hero{background:linear-gradient(135deg,rgba(16,42,67,.97),rgba(184,92,0,.94));padding:3rem 2rem;border-radius:24px;color:white;text-align:center;margin-bottom:1.5rem;box-shadow:0 20px 50px rgba(16,42,67,.18);}
.uh-hero h1{font-size:2.6rem;margin-bottom:.2rem}.uh-hero p{font-size:1.15rem;opacity:.95}
.uh-card{background:linear-gradient(145deg,rgba(255,255,255,.62),rgba(255,255,255,.18));border:1px solid rgba(255,255,255,.85);border-radius:20px;padding:1.2rem;box-shadow:inset 0 1px 0 rgba(255,255,255,.9),0 12px 30px rgba(16,42,67,.08);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);margin-bottom:.8rem;}
.uh-badge{padding:3px 10px;border-radius:999px;font-size:.8rem;font-weight:700}.uh-demo-banner{background:rgba(255,244,229,.88);border:1px solid #ffb800;color:#7a4b00;padding:.6rem 1rem;border-radius:12px;font-weight:700;margin-bottom:1rem;}
div[data-baseweb="input"]>div,div[data-baseweb="textarea"]>div,div[data-baseweb="select"]>div,.stDateInput>div>div,.stNumberInput>div>div{background:linear-gradient(145deg,rgba(255,255,255,.40),rgba(255,255,255,.12))!important;border:1px solid rgba(255,255,255,.9)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.9),0 8px 22px rgba(16,42,67,.07)!important;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border-radius:14px!important;}
div[data-baseweb="input"] input,div[data-baseweb="textarea"] textarea{background:transparent!important;color:#102a43!important;}
label,.stMarkdown,.stText,p,span{color:#102a43;}
.stButton>button{border-radius:14px;border:1px solid rgba(255,255,255,.9);background:linear-gradient(145deg,rgba(255,255,255,.72),rgba(255,255,255,.28));color:#102a43;font-weight:700;box-shadow:0 8px 22px rgba(16,42,67,.08),inset 0 1px 0 rgba(255,255,255,.95);}
.stButton>button:hover{transform:translateY(-1px);box-shadow:0 12px 26px rgba(16,42,67,.12)}
[data-testid="stMetric"]{background:rgba(255,255,255,.30);border:1px solid rgba(255,255,255,.72);border-radius:18px;padding:.7rem;box-shadow:inset 0 1px 0 rgba(255,255,255,.8),0 10px 25px rgba(16,42,67,.06);}
</style>
"""
...
def render_landing():
    ...
    with c2:
        st.markdown('<div class="uh-card"><h3>🤝 Borrowing</h3>Find campus items available to borrow — securely.</div>', unsafe_allow_html=True)
    with c3:
...
def render_register():
    ...
    st.caption("Use any valid email address. Your account will be activated after email verification and admin approval.")
        ...
        if st.button("Verify"):
            ...
            if not u:
                ...
                if ok:
                    ...
                    conn.commit()
                    conn.close()
                    notify(u["id"], "Your email has been verified. Your account is now waiting for admin approval.")
                    st.success("Email verified! Your account is now waiting for admin approval.")
                    st.session_state["auth_mode"] = "Login"
                    st.rerun()
                else:
                    st.error(msg)
...
def render_dashboard(user):
    ...
    elif user["role"] == "student" and not user["admin_approved"]:
...
def render_borrowing(user):
    ...
    tabs = st.tabs(["Browse Items", "My Items & Requests", "My Borrowings"])

    with tabs[0]:
        st.markdown("#### Available items to borrow")
        conn = get_conn()
        avail = conn.execute(
        ...
        for it in avail:
            with st.container(border=True):
                ...
                with cols[1]:
                    ...
                    if st.button("🤝 Request to Borrow", key=f"reqborrow_{it['id']}", use_container_width=True):
                        conn = get_conn()
                        check = conn.execute("SELECT status FROM items WHERE id=?", (it["id"],)).fetchone()
                        existing = conn.execute(
                            "SELECT id FROM borrowings WHERE item_id=? AND borrower_id=? AND status IN ('REQUESTED','ACCEPTED','ACTIVE','RETURN_REQUESTED')",
                            (it["id"], user["id"]),
                        ).fetchone()
                        if not check or check["status"] != "AVAILABLE":
                            ...
                            st.warning("You already have an active request for this item.")
                            conn.close()
                        else:
                            conn.execute(
                                """INSERT INTO borrowings (item_id, owner_id, borrower_id, status, expected_return, deposit, created_at)
                                   VALUES (?,?,?, 'REQUESTED', ?, ?, ?)""",
                            ...
                            st.success("Borrow request sent. The owner can accept or reject it.")
                            st.rerun()

    with tabs[1]:
        st.markdown("#### My items & incoming borrow requests")
        conn = get_conn()
        my_items = conn.execute("SELECT * FROM items WHERE owner_id=? ORDER BY id DESC", (user["id"],)).fetchall()
        conn.close()
        if not my_items:
            st.info("You don't have any listed items. Existing campus items can still be requested from Browse Items.")
        for it in my_items:
            with st.container(border=True):
                st.markdown(f"**{it['item_name']}**  {status_badge(it['status'])}", unsafe_allow_html=True)
                conn = get_conn()
                reqs = conn.execute(
                    "SELECT b.*, u.full_name borrower_name, u.phone borrower_phone, u.email borrower_email "
                    "FROM borrowings b JOIN users u ON u.id=b.borrower_id WHERE b.item_id=? ORDER BY b.id DESC", (it["id"],)
                ).fetchall()
                conn.close()
                if not reqs:
                    st.caption("No borrow requests for this item.")
                for b in reqs:
                    st.markdown(f"**Request from {b['borrower_name']}**  {status_badge(b['status'])}", unsafe_allow_html=True)
                    st.caption(f"Expected return: {b['expected_return'] or 'Not specified'}")
                    if b["status"] == "REQUESTED":
                        st.info("📥 Borrow request received — accept or reject it here.")
                        c1, c2 = st.columns(2)
                        if c1.button("✅ Accept Request", key=f"bacc_{b['id']}", use_container_width=True):
                            conn = get_conn()
                            updated = conn.execute("UPDATE borrowings SET status='ACCEPTED' WHERE id=? AND status='REQUESTED'", (b["id"],))
                            if updated.rowcount == 1:
                                conn.execute("UPDATE items SET status='BORROWED' WHERE id=? AND status='AVAILABLE'", (it["id"],))
                                conn.commit(); conn.close()
                                if b["deposit"] > 0:
                                ...
                                st.success("Borrow request accepted.")
                                st.rerun()
                            else:
                                conn.rollback(); conn.close(); st.error("This request was already processed.")
                        if c2.button("❌ Reject Request", key=f"brej_{b['id']}", use_container_width=True):
                            conn = get_conn()
                            conn.execute("UPDATE borrowings SET status='REJECTED' WHERE id=? AND status='REQUESTED'", (b["id"],))
                            conn.commit(); conn.close()
                            notify(b["borrower_id"], f"Your request to borrow {it['item_name']} was rejected.")
                            st.rerun()
                    elif b["status"] == "ACCEPTED":
                        st.caption("Waiting for the borrower to verify pickup with your OTP/QR.")
                        if b["borrower_phone"] or b["borrower_email"]:
                            links=[]
                            if b["borrower_phone"]: links.append(f"[📞 Call](tel:{b['borrower_phone']})")
                            if b["borrower_email"]: links.append(f"[✉️ Email](mailto:{b['borrower_email']})")
                            st.markdown("**Contact borrower:** " + " · ".join(links))
                        if st.button("🔐 Generate Pickup OTP", key=f"bgenotp_{b['id']}"):
                            deliver_otp(dict(user), "PICKUP_VERIFICATION", 100000 + b["id"], f"pickup of {it['item_name']}")
                        if st.button("🔳 Generate Pickup QR", key=f"bgenqr_{b['id']}"):
                            token = create_qr_token("BORROWING", b["id"], "PICKUP_VERIFICATION")
                            st.image(generate_qr_image_bytes(token), width=180)
                            st.caption(f"Manual fallback code: `{token}`")
                    elif b["status"] == "ACTIVE":
                        st.caption("Item is currently with the borrower.")
                        if st.button("🔳 Generate Return QR", key=f"brgenqr_{b['id']}"):
                            token = create_qr_token("BORROWING", b["id"], "RETURN_VERIFICATION")
                            st.image(generate_qr_image_bytes(token), width=180)
                            st.caption(f"Manual fallback code: `{token}`")
                    elif b["status"] == "RETURN_REQUESTED":
                        st.caption("Borrower requested return. Generate the return OTP for verification.")
                        if st.button("🔐 Generate Return OTP", key=f"bgenretotp_{b['id']}"):
                            deliver_otp(dict(user), "RETURN_VERIFICATION", 100000 + b["id"], f"return of {it['item_name']}")
                    if b["status"] == "COMPLETED":
                        st.success("Borrowing completed.")
    ...
    with tabs[2]:
        ...
        conn = get_conn()
        mine = conn.execute(
            "SELECT b.*, i.item_name, u.full_name owner_name, u.phone owner_phone, u.email owner_email FROM borrowings b "
            "JOIN items i ON i.id=b.item_id JOIN users u ON u.id=b.owner_id WHERE b.borrower_id=? ORDER BY b.id DESC", (user["id"],)
        ).fetchall()
        conn.close()
        if not mine:
        ...
        for b in mine:
            with st.container(border=True):
                ...
                if b["status"] in ("ACCEPTED", "ACTIVE", "RETURN_REQUESTED") and (b["owner_phone"] or b["owner_email"]):
                    links=[]
                    if b["owner_phone"]: links.append(f"[📞 Call](tel:{b['owner_phone']})")
                    if b["owner_email"]: links.append(f"[✉️ Email](mailto:{b['owner_email']})")
                    st.markdown("**Contact owner:** " + " · ".join(links))
                if b["status"] == "ACCEPTED":
                    ...
                    if m == "Enter OTP":
                        ...
                        if st.button("Verify pickup OTP", key=f"bpverify_{b['id']}"):
                            ...
                            if ok: _finalize_borrow_pickup(b)
                            else: st.error(msg)
                    else:
                    ...
                    photo = st.file_uploader("Item condition photo at pickup (optional)", type=["png","jpg","jpeg"], key=f"bcs_{b['id']}")
                    if photo is not None and st.button("Save condition photo", key=f"bcs_save_{b['id']}"):
                        fname=f"borrow_{b['id']}_start_{int(time.time())}_{photo.name}"; fpath=os.path.join(PHOTOS_DIR,fname)
                        with open(fpath,"wb") as f: f.write(photo.getbuffer())
                        conn=get_conn(); conn.execute("UPDATE borrowings SET condition_photo_start=? WHERE id=?",(fpath,b['id'])); conn.commit(); conn.close(); st.success("Condition photo saved.")
                if b["status"] == "ACTIVE":
                    if st.button("↩ Request Return", key=f"reqret_{b['id']}"):
                        conn=get_conn(); conn.execute("UPDATE borrowings SET status='RETURN_REQUESTED' WHERE id=? AND status='ACTIVE'",(b['id'],)); conn.commit(); conn.close(); notify(b['owner_id'],f"{user['full_name']} wants to return {b['item_name']}."); st.rerun()
...
def get_active_helper_task(user_id):
    conn = get_conn()
    row = conn.execute(
        "SELECT t.*, u.full_name creator_name, u.phone creator_phone, u.email creator_email "
...
def task_code(task_id):
    """Compact public task ID format, e.g. UNIH9877."""
    return f"UNIH{int(task_id):04d}"


def make_new_task_code(conn):
    """Generate a short unique UNIH#### task code."""
    for _ in range(50):
        code = f"UNIH{secrets.randbelow(10000):04d}"
        if conn.execute("SELECT 1 FROM tasks WHERE task_code=?", (code,)).fetchone() is None:
            return code
    return task_code(conn.execute("SELECT COALESCE(MAX(id),0)+1 id FROM tasks").fetchone()["id"])


def render_microtasks(user):
    st.markdown("## 🛠 Micro-Tasks")

    # A helper with an accepted/in-progress task is locked to that task until it is finished.
    active = get_active_helper_task(user["id"])
    if active:
        st.markdown("### 🔒 Your active task")
        st.caption(f"Task ID: **{active['task_code'] or task_code(active['id'])}**")
        with st.container(border=True):
            st.markdown(f"**{active['title']}**  {status_badge(active['status'])}", unsafe_allow_html=True)
            st.write(active["description"] or "")
            st.write(f"💰 ₹{active['reward']:.0f}  |  👤 Requester: {active['creator_name']}")
            if active["creator_phone"] or active["creator_email"]:
                links = []
                if active["creator_phone"]:
                    links.append(f"[📞 Call](tel:{active['creator_phone']})")
                if active["creator_email"]:
                    links.append(f"[✉️ Email](mailto:{active['creator_email']})")
                st.markdown("**Contact requester:** " + " · ".join(links))
            if active["status"] == "ACCEPTED":
                if st.button("▶ Start Task", key=f"active_start_locked_{active['id']}", use_container_width=True):
                    conn = get_conn()
                    conn.execute("UPDATE tasks SET status='IN_PROGRESS' WHERE id=? AND helper_id=? AND status='ACCEPTED'", (active['id'], user['id']))
                    conn.commit(); conn.close(); st.rerun()
            elif active["status"] == "IN_PROGRESS":
                if active["admin_otp_bypass"]:
                    st.success("🛡 Admin verification override enabled — OTP is not required.")
                    if st.button("✅ Complete Task", key=f"active_complete_admin_{active['id']}", use_container_width=True):
                        conn = get_conn()
                        conn.execute("UPDATE tasks SET status='COMPLETED', completed_at=? WHERE id=? AND helper_id=? AND status='IN_PROGRESS'", (now_iso(), active['id'], user['id']))
                        conn.commit(); conn.close()
                        update_transaction_status("TASK", active['id'], "RELEASED")
                        add_unicoins(user['id'], 15, "Micro-task completed")
                        recalc_trust_score(user['id'])
                        notify(active['creator_id'], f"Your task '{active['title']}' was completed by {user['full_name']}.")
                        st.success("Task completed!")
                        st.rerun()
                else:
                    code = st.text_input("Completion OTP from requester", key=f"active_complete_code_{active['id']}", max_chars=6)
                    if st.button("Verify & Complete", key=f"active_complete_btn_{active['id']}", use_container_width=True):
                        ok, msg = verify_otp(active['creator_id'], "DELIVERY_COMPLETION", active['id'], code)
                        if ok:
                            conn = get_conn()
                            conn.execute("UPDATE tasks SET status='COMPLETED', completed_at=? WHERE id=? AND helper_id=? AND status='IN_PROGRESS'", (now_iso(), active['id'], user['id']))
                            conn.commit(); conn.close()
                            ...
                            add_unicoins(user['id'], 15, "Micro-task completed")
                            recalc_trust_score(user['id'])
                            notify(active['creator_id'], f"Your task '{active['title']}' was completed and verified.")
                            st.success("Task completed!")
                            st.rerun()
                        else:
                            st.error(msg)
        st.info("🔒 UNI HELP is locked to your active task until it is completed.")
        return
        ...
        with st.form("create_task"):
            ...
            if submitted:
                if not title.strip():
                    ...
                    conn = get_conn()
                    public_code = make_new_task_code(conn)
                    conn.execute(
                        """INSERT INTO tasks (creator_id, title, description, pickup, destination,
                            reward, deadline, category, status, created_at, task_code)
                           VALUES (?,?,?,?,?,?,?,?, 'CREATED', ?, ?)""",
                        (user["id"], title.strip(), description, pickup, destination, reward, deadline, category, now_iso(), public_code),
                    )
    ...
    with tabs[1]:
        ...
        for t in open_tasks:
            with st.container(border=True):
                ...
                st.caption(f"Task ID: **{t['task_code'] or task_code(t['id'])}**")
                st.write(t["description"] or "")
                st.write(f"💰 ₹{t['reward']:.0f}  |  ⏰ {t['deadline'] or 'Flexible'}")
                if st.button("✅ Accept Task", key=f"tacc_{t['id']}"):
                    conn = get_conn()
                    active_check = conn.execute("SELECT id FROM tasks WHERE helper_id=? AND status IN ('ACCEPTED','IN_PROGRESS') LIMIT 1", (user['id'],)).fetchone()
                    check = conn.execute("SELECT status FROM tasks WHERE id=?", (t['id'],)).fetchone()
                    if active_check:
                        st.error("You already have an active task. Complete it before accepting another.")
                        conn.close()
                    elif not check or check["status"] != "CREATED":
                        st.error("This task is no longer available.")
                        conn.close()
                    else:
                        updated = conn.execute("UPDATE tasks SET helper_id=?, status='ACCEPTED', accepted_at=? WHERE id=? AND status='CREATED'", (user["id"], now_iso(), t["id"]))
                        conn.commit()
                        conn.close()
                        if updated.rowcount == 1:
                            create_transaction(t["creator_id"], user["id"], "TASK", t["id"], t["reward"], "HELD")
                            notify(t["creator_id"], f"{user['full_name']} accepted your task: {t['title']} (Task ID {t['task_code'] or task_code(t['id'])}).")
                            st.rerun()
                        else:
                            st.error("This task is no longer available.")
    ...
    with tabs[3]:
        ...
        for t in mine:
            with st.container(border=True):
                ...
                st.caption(f"Task ID: **{t['task_code'] or task_code(t['id'])}**")
                creator = user_by_id(t["creator_id"])
                if creator and creator["phone"]:
                    st.markdown(f"📞 **Contact requester:** [Call {creator['full_name']}](tel:{creator['phone']})")
...
def delete_user_account(conn, user_id):
    """Permanently delete one student and the app records owned by that student.
    Admin accounts are never deleted through this function."""
    target = conn.execute("SELECT role FROM users WHERE id=?", (user_id,)).fetchone()
    if not target:
        raise ValueError("User not found")
    if target["role"] == "admin":
        raise ValueError("Admin accounts cannot be deleted from student management.")

    request_ids = [r[0] for r in conn.execute("SELECT id FROM requests WHERE requester_id=? OR helper_id=?", (user_id, user_id)).fetchall()]
    task_ids = [r[0] for r in conn.execute("SELECT id FROM tasks WHERE creator_id=? OR helper_id=?", (user_id, user_id)).fetchall()]
    item_ids = [r[0] for r in conn.execute("SELECT id FROM items WHERE owner_id=?", (user_id,)).fetchall()]
    borrowing_ids = [r[0] for r in conn.execute("SELECT id FROM borrowings WHERE owner_id=? OR borrower_id=? OR item_id IN ({})".format(','.join('?'*len(item_ids)) if item_ids else 'NULL'), tuple([user_id,user_id]+item_ids)).fetchall()] if item_ids else [r[0] for r in conn.execute("SELECT id FROM borrowings WHERE owner_id=? OR borrower_id=?", (user_id,user_id)).fetchall()]

    # Remove dependent records first because the schema uses foreign keys.
    if request_ids:
        marks=','.join('?'*len(request_ids))
        conn.execute(f"DELETE FROM transactions WHERE related_type='DELIVERY' AND related_id IN ({marks})", request_ids)
        conn.execute(f"DELETE FROM disputes WHERE transaction_type='DELIVERY' AND transaction_id IN ({marks})", request_ids)
        conn.execute(f"DELETE FROM locations WHERE reference_type='REQUEST' AND reference_id IN ({marks})", request_ids)
        conn.execute(f"DELETE FROM qr_tokens WHERE reference_type='REQUEST' AND reference_id IN ({marks})", request_ids)
        conn.execute(f"DELETE FROM requests WHERE id IN ({marks})", request_ids)
    if task_ids:
        marks=','.join('?'*len(task_ids))
        conn.execute(f"DELETE FROM transactions WHERE related_type='TASK' AND related_id IN ({marks})", task_ids)
        conn.execute(f"DELETE FROM disputes WHERE transaction_type='TASK' AND transaction_id IN ({marks})", task_ids)
        conn.execute(f"DELETE FROM locations WHERE reference_type='TASK' AND reference_id IN ({marks})", task_ids)
        conn.execute(f"DELETE FROM qr_tokens WHERE reference_type='TASK' AND reference_id IN ({marks})", task_ids)
        conn.execute(f"DELETE FROM tasks WHERE id IN ({marks})", task_ids)
    if borrowing_ids:
        marks=','.join('?'*len(borrowing_ids))
        conn.execute(f"DELETE FROM transactions WHERE related_type='BORROWING' AND related_id IN ({marks})", borrowing_ids)
        conn.execute(f"DELETE FROM disputes WHERE transaction_type='BORROWING' AND transaction_id IN ({marks})", borrowing_ids)
        conn.execute(f"DELETE FROM locations WHERE reference_type='BORROWING' AND reference_id IN ({marks})", borrowing_ids)
        conn.execute(f"DELETE FROM qr_tokens WHERE reference_type='BORROWING' AND reference_id IN ({marks})", borrowing_ids)
        conn.execute(f"DELETE FROM borrowings WHERE id IN ({marks})", borrowing_ids)
    if item_ids:
        marks=','.join('?'*len(item_ids))
        conn.execute(f"DELETE FROM items WHERE id IN ({marks})", item_ids)

    conn.execute("DELETE FROM ratings WHERE rater_id=? OR ratee_id=?", (user_id,user_id))
    conn.execute("DELETE FROM notifications WHERE user_id=?", (user_id,))
    conn.execute("DELETE FROM otp_records WHERE user_id=?", (user_id,))
    conn.execute("DELETE FROM unicoin_transactions WHERE user_id=?", (user_id,))
    conn.execute("DELETE FROM transactions WHERE payer_id=? OR payee_id=?", (user_id,user_id))
    conn.execute("DELETE FROM disputes WHERE reporter_id=?", (user_id,))
    conn.execute("DELETE FROM admin_actions WHERE target_id=?", (user_id,))
    conn.execute("DELETE FROM users WHERE id=? AND role='student'", (user_id,))
    if conn.total_changes < 1:
        raise ValueError("Student account could not be deleted.")
...
def render_admin(user):
    ...
    approved_users = conn.execute("SELECT COUNT(*) c FROM users WHERE role='student' AND verified=1 AND admin_approved=1").fetchone()["c"]
    ...
    r1[1].metric("Email Verified", verified_users)
    r1[2].metric("Approved Users", approved_users)
    r1[3].metric("Active Requests", active_requests)
    r2 = st.columns(4)
    ...
    st.divider()
    tabs = st.tabs(["Users", "Broadcast", "Disputes", "Transactions", "Requests", "Tasks", "Maintenance"])

    with tabs[0]:
        st.markdown("### 👥 User management")
        st.caption("Admin controls: resend verification email, approve verified students, suspend/unsuspend, or delete accounts.")
        user_filter = st.selectbox(
            "Show users",
            ["All", "Pending email verification", "Pending admin approval", "Verified & approved", "Suspended"],
            key="admin_user_filter",
        )
        if user_filter == "Pending email verification":
            users = conn.execute("SELECT * FROM users WHERE role='student' AND verified=0 ORDER BY id DESC").fetchall()
        elif user_filter == "Pending admin approval":
            users = conn.execute("SELECT * FROM users WHERE role='student' AND verified=1 AND admin_approved=0 ORDER BY id DESC").fetchall()
        elif user_filter == "Verified & approved":
            users = conn.execute("SELECT * FROM users WHERE role='student' AND verified=1 AND admin_approved=1 ORDER BY id DESC").fetchall()
        elif user_filter == "Suspended":
            users = conn.execute("SELECT * FROM users WHERE role='student' AND is_suspended=1 ORDER BY id DESC").fetchall()
        else:
        ...
        for u in users:
            with st.container(border=True):
                status = "🟢 VERIFIED + APPROVED" if u["verified"] and u["admin_approved"] else ("🟡 EMAIL VERIFIED — APPROVAL PENDING" if u["verified"] else "🔴 EMAIL VERIFICATION PENDING")
                st.markdown(f"### {u['full_name']}")
                st.write(f"📧 {u['email']}  ·  Student ID: {u['student_id'] or '—'}")
                st.write(f"**Status:** {status}  ·  **Trust:** {u['trust_score']}  ·  **Joined:** {u['created_at']}")

                a, b, c, d = st.columns(4)
                if not u["verified"]:
                    if a.button("📧 Send verification", key=f"admin_verify_{u['id']}", use_container_width=True):
                        target = user_by_id(u["id"])
                        if target:
                            deliver_otp(target, "EMAIL_VERIFICATION", None, "email verification")
                            conn.execute("INSERT INTO admin_actions (admin_id, action, target_id, details, created_at) VALUES (?,?,?,?,?)",
                                         (user["id"], "SEND_VERIFICATION_EMAIL", u["id"], "Admin resent email verification code", now_iso()))
                            conn.commit()
                else:
                    a.button("✓ Email verified", key=f"email_verified_{u['id']}", disabled=True, use_container_width=True)

                if u["verified"] and not u["admin_approved"]:
                    if b.button("✅ Approve account", key=f"approve_{u['id']}", use_container_width=True):
                        conn.execute("UPDATE users SET admin_approved=1 WHERE id=? AND role='student'", (u["id"],))
                        conn.execute("INSERT INTO admin_actions (admin_id, action, target_id, details, created_at) VALUES (?,?,?,?,?)",
                                     (user["id"], "APPROVE_USER", u["id"], "Admin approved student account", now_iso()))
                        conn.commit()
                        notify(u["id"], "🎉 Your UNI HELP account has been approved by an administrator. You can now log in.")
                        st.success(f"{u['full_name']} approved.")
                        st.rerun()
                else:
                    b.button("✓ Approved", key=f"approved_{u['id']}", disabled=True, use_container_width=True)

                if u["is_suspended"]:
                    if c.button("🔓 Unsuspend", key=f"unsusp_{u['id']}", use_container_width=True):
                    ...
                    if c.button("⛔ Suspend", key=f"susp_{u['id']}", use_container_width=True):
                ...
                if d.button("🗑 Delete account", key=f"delete_user_{u['id']}", use_container_width=True):
                    st.session_state[f"confirm_delete_user_{u['id']}"] = True
                    st.rerun()

                if st.session_state.get(f"confirm_delete_user_{u['id']}"):
                    st.warning(f"Delete {u['full_name']}'s account permanently? This removes their account and related app records.")
                    y, n = st.columns(2)
                    if y.button("Yes, permanently delete", key=f"confirm_yes_{u['id']}", type="primary", use_container_width=True):
                        try:
                            delete_user_account(conn, u["id"])
                            conn.execute("INSERT INTO admin_actions (admin_id, action, target_id, details, created_at) VALUES (?,?,?,?,?)",
                                         (user["id"], "DELETE_USER", u["id"], f"Deleted account {u['email']}", now_iso()))
                            conn.commit()
                            st.session_state.pop(f"confirm_delete_user_{u['id']}", None)
                            st.success("Account deleted.")
                            st.rerun()
                        except sqlite3.IntegrityError as e:
                            conn.rollback()
                            st.error(f"Could not delete this account because related records are still protected: {e}")
                    if n.button("Cancel", key=f"confirm_no_{u['id']}", use_container_width=True):
                        st.session_state.pop(f"confirm_delete_user_{u['id']}", None)
                        st.rerun()

    with tabs[1]:
        st.markdown("### 📢 Send announcement")
        st.caption("Send one message to every student. It appears in UNI HELP Notifications and, if SMTP is configured, by email too.")
        with st.form("admin_broadcast_form"):
            subject = st.text_input("Subject", placeholder="UNI HELP announcement")
            message = st.text_area("Message", height=160, placeholder="Write the message for all students.")
            send = st.form_submit_button("📢 Send to all students", use_container_width=True)
            if send:
                if not subject.strip() or not message.strip():
                    st.error("Subject and message are required.")
                else:
                    total, sent, failed = broadcast_to_all_students(subject.strip(), message.strip())
                    conn.execute("INSERT INTO admin_actions (admin_id, action, target_id, details, created_at) VALUES (?,?,?,?,?)", (user["id"], "BROADCAST_ANNOUNCEMENT", None, f"Students: {total}; emails sent: {sent}; failures: {failed}", now_iso()))
                    conn.commit()
                    if EMAIL_CONFIGURED:
                        st.success(f"📢 Sent to {total} students. Email sent: {sent}; failed: {failed}.")
                    else:
                        st.success(f"📢 In-app notification sent to {total} students. SMTP is not configured, so no real email was sent.")
    ...
    with tabs[5]:
        ...
        task_query = st.text_input("Search Task ID", placeholder="e.g. UNIH9877", key="admin_task_id")
        if task_query.strip():
            row = conn.execute(
                "SELECT t.*, cu.full_name creator_name, hu.full_name helper_name FROM tasks t "
                "JOIN users cu ON cu.id=t.creator_id LEFT JOIN users hu ON hu.id=t.helper_id "
                "WHERE UPPER(t.task_code)=UPPER(?)", (task_query.strip(),)
            ).fetchone()
            if not row:
                st.warning("No task found with this Task ID.")
            else:
                st.markdown(f"### Task **{row['task_code'] or task_code(row['id'])}**")
                st.write(f"**{row['title']}** — {status_badge(row['status'])}", unsafe_allow_html=True)
                st.write(f"Requester: {row['creator_name']}  |  Helper: {row['helper_name'] or 'Not assigned'}")
                st.write(f"Reward: ₹{row['reward']:.0f}  |  Category: {row['category'] or 'Other'}")
                if row['status'] in ('ACCEPTED', 'IN_PROGRESS'):
                    if row['admin_otp_bypass']:
                        st.success("🛡 OTP-less completion is enabled. The assigned helper still completes the task.")
                    elif st.button("🛡 Allow OTP-less completion", key=f"admin_bypass_{row['id']}"):
                        conn.execute("UPDATE tasks SET admin_otp_bypass=1 WHERE id=? AND status IN ('ACCEPTED','IN_PROGRESS')", (row['id'],))
                        conn.execute("INSERT INTO admin_actions (admin_id, action, target_id, details, created_at) VALUES (?,?,?,?,?)",
                                     (user['id'], 'TASK_OTP_BYPASS', row['id'], f"Task {row['task_code'] or task_code(row['id'])}: admin enabled OTP-less completion", now_iso()))
                        conn.commit()
                        notify(row['creator_id'], f"Admin enabled OTP-less completion for task {row['task_code'] or task_code(row['id'])}.")
                        st.success("OTP-less completion enabled. The admin did not complete the task; the assigned helper must still complete it.")
                        st.rerun()
                else:
                    st.caption("OTP-less completion can be enabled only for an accepted/in-progress task.")

    with tabs[6]:
        st.markdown("### 🧹 Database maintenance")
        st.warning("These controls remove all micro-task records or all borrowing request records. Users and delivery requests are not deleted.")
        c1,c2=st.columns(2)
        if c1.button("🗑 Clear all tasks",use_container_width=True): st.session_state["confirm_clear_tasks"]=True
        if c2.button("🗑 Clear all borrow requests",use_container_width=True): st.session_state["confirm_clear_borrowings"]=True
        if st.session_state.get("confirm_clear_tasks"):
            st.error("Delete ALL tasks and their related task transactions/disputes? This cannot be undone.")
            y,n=st.columns(2)
            if y.button("Yes, clear ALL tasks",type="primary",use_container_width=True):
                ids=[r[0] for r in conn.execute("SELECT id FROM tasks").fetchall()]
                if ids:
                    marks=",".join("?"*len(ids))
                    conn.execute(f"DELETE FROM transactions WHERE related_type='TASK' AND related_id IN ({marks})",ids)
                    conn.execute(f"DELETE FROM disputes WHERE transaction_type='TASK' AND transaction_id IN ({marks})",ids)
                    conn.execute(f"DELETE FROM locations WHERE reference_type='TASK' AND reference_id IN ({marks})",ids)
                    conn.execute(f"DELETE FROM qr_tokens WHERE reference_type='TASK' AND reference_id IN ({marks})",ids)
                conn.execute("DELETE FROM tasks")
                conn.execute("INSERT INTO admin_actions (admin_id, action, target_id, details, created_at) VALUES (?,?,?,?,?)",(user["id"],"CLEAR_ALL_TASKS",None,f"Deleted {len(ids)} tasks",now_iso()))
                conn.commit(); st.session_state.pop("confirm_clear_tasks",None); st.success(f"Cleared {len(ids)} task(s)."); st.rerun()
            if n.button("Cancel",use_container_width=True): st.session_state.pop("confirm_clear_tasks",None); st.rerun()
        if st.session_state.get("confirm_clear_borrowings"):
            st.error("Delete ALL borrowing request records, including active/completed records? This cannot be undone.")
            y,n=st.columns(2)
            if y.button("Yes, clear ALL borrow requests",type="primary",use_container_width=True):
                ids=[r[0] for r in conn.execute("SELECT id FROM borrowings").fetchall()]
                if ids:
                    marks=",".join("?"*len(ids))
                    conn.execute(f"DELETE FROM transactions WHERE related_type='BORROW_DEPOSIT' AND related_id IN ({marks})",ids)
                    conn.execute(f"DELETE FROM transactions WHERE related_type='BORROWING' AND related_id IN ({marks})",ids)
                    conn.execute(f"DELETE FROM disputes WHERE transaction_type='BORROWING' AND transaction_id IN ({marks})",ids)
                    conn.execute(f"DELETE FROM locations WHERE reference_type='BORROWING' AND reference_id IN ({marks})",ids)
                    conn.execute(f"DELETE FROM qr_tokens WHERE reference_type='BORROWING' AND reference_id IN ({marks})",ids)
                conn.execute("DELETE FROM borrowings"); conn.execute("UPDATE items SET status='AVAILABLE'")
                conn.execute("INSERT INTO admin_actions (admin_id, action, target_id, details, created_at) VALUES (?,?,?,?,?)",(user["id"],"CLEAR_ALL_BORROWINGS",None,f"Deleted {len(ids)} borrowing requests",now_iso()))
                conn.commit(); st.session_state.pop("confirm_clear_borrowings",None); st.success(f"Cleared {len(ids)} borrowing request(s)."); st.rerun()
            if n.button("Cancel",use_container_width=True): st.session_state.pop("confirm_clear_borrowings",None); st.rerun()
...
def main():
    ...
    # Server-side access gate: students cannot remain logged in unless both
    # email verification and admin approval are complete.
    if user["role"] == "student" and (not user["verified"] or not user["admin_approved"]):
        st.session_state["user"] = None
        st.session_state["auth_mode"] = "Login"
        if not user["verified"]:
            st.error("🔒 Your email verification is still pending. Please verify your email first.")
        else:
            st.error("🔒 Your account is still waiting for admin approval.")
        st.stop()
